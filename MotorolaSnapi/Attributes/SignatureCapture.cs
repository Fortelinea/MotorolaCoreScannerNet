...
namespace Motorola.Snapi.Attributes
{
    ...
    public class SignatureCapture : MotorolaAttributeSet
    {
        ...
        {
            get { return Convert.ToUInt16(GetAttribute((ushort)SignatureCaptureAttribute.CaptureHeight).Value); }
            set
            {
                SetAttribute(new ScannerAttribute
                             {
                                 Id = (ushort)SignatureCaptureAttribute.CaptureHeight,
        ...
        {
            get { return Convert.ToUInt16(GetAttribute((ushort)SignatureCaptureAttribute.CaptureWidth).Value); }
            set
            {
                SetAttribute(new ScannerAttribute
                             {
                                 Id = (ushort)SignatureCaptureAttribute.CaptureWidth,
                                 DataType = DataType.Byte,
                                 Value = value
                             });
            }
        ...
        ///     <remarks>
        ///         <para>NOTE: Scanning an signature capture bar code will product an Image of the contents within the bar code.</para>
        ///     </remarks>
        /// </summary>
        public bool SignatureCaptureEnabled
        {
            get { return (bool)GetAttribute((ushort)SignatureCaptureAttribute.SignatureCapture).Value; }
            set
            {
                SetAttribute(new ScannerAttribute
                             {
                                 Id = (ushort)SignatureCaptureAttribute.SignatureCapture,
